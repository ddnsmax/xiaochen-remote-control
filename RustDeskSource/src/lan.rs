...
static CONTROLLER_SEEN: AtomicBool = AtomicBool::new(false);
static CONTROLLER_LAST_SEEN: AtomicU64 = AtomicU64::new(0);
...
pub fn controller_status() -> i32 {
    if !CONTROLLER_SEEN.load(Ordering::Relaxed) {
        return 0;
    }
    if now_millis().saturating_sub(CONTROLLER_LAST_SEEN.load(Ordering::Relaxed)) <= 15_000 {
        1
    } else {
        2
    }
            ...
            if let Ok(msg_in) = Message::parse_from_bytes(&buf[0..len]) {
                match msg_in.union {
                    Some(rendezvous_message::Union::PeerDiscovery(p)) => {
                        if p.cmd == "ack"
                            && crate::common::is_lan_v1_controller_ip(addr.ip())
                            && p.id == Config::get_id()
                        {
                            CONTROLLER_SEEN.store(true, Ordering::Relaxed);
                            CONTROLLER_LAST_SEEN.store(now_millis(), Ordering::Relaxed);
                        } else if p.cmd == "ping"
                            && (crate::common::is_lan_v1_role()
                                || config::option2bool(
                ...
                match msg_in.union {
                    Some(rendezvous_message::Union::PeerDiscovery(p)) => {
                        ...
                        if p.cmd == "pong" {
                            ...
                            if crate::common::lan_v1_role() == "controller"
                                || local_mac.is_empty() && p.mac.is_empty()
                                || local_mac != p.mac
                            {
                                let mut msg_out = Message::new();
                                msg_out.set_peer_discovery(PeerDiscovery {
                                    cmd: "ack".to_owned(),
                                    id: p.id.clone(),
                                    ..Default::default()
                                });
                                allow_err!(socket.send_to(&msg_out.write_to_bytes()?, addr));
