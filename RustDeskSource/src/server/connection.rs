...
impl Connection {
    pub async fn start(
        ...
        conn.lan_direct_full_access =
            lan_direct && crate::common::is_lan_v1_controller_ip(addr.ip());
        if conn.lan_direct_full_access {
            conn.keyboard = true;
            conn.clipboard = true;
            conn.audio = true;
            conn.file = true;
            ...
            tokio::select! {
                ...
                Some(data) = rx_from_cm.recv() => {
                    match data {
                        ...
                        ipc::Data::Close => {
                            log::info!("Ignore embedded connection-manager close request");
                            continue;
                        }
                        ipc::Data::CmErr(e) => {
                            if e != "expected" {
                        ...
                        ipc::Data::DataPortableService(ipc::DataPortableService::RequestStart) => {
                            if let Err(e) = portable_client::start_portable_service(
    ...
    fn normalize_conn_audit_auth_fields(&mut self) {
        if matches!(
            ...
            ConnAuditPrimaryAuth::Click
                | ConnAuditPrimaryAuth::SwitchSides
                | ConnAuditPrimaryAuth::LanDirect
        ) {
    ...
    fn try_start_cm_ipc(&mut self) {
        if let Some(p) = self.start_cm_ipc_para.take() {
            ...
            std::thread::spawn(move || {
                if crate::common::lan_v1_role() != "host"
                    && crate::is_server()
        ...
        if let Some(message::Union::LoginRequest(lr)) = msg.union {
            ...
            match lr.union {
                Some(login_request::Union::FileTransfer(ft)) => {
                    if !self.lan_direct_full_access
                        && !Self::permission(
                ...
                Some(login_request::Union::ViewCamera(_vc)) => {
                    if !self.lan_direct_full_access
                        && !Self::permission(keys::OPTION_ENABLE_CAMERA, &self.control_permissions)
                    {
            ...
            if self.lan_direct_full_access {
                self.set_conn_audit_primary_auth(ConnAuditPrimaryAuth::LanDirect);
    ...
    fn portable_check(&mut self) {
        ...
        let show_elevation = !self.lan_direct_full_access && !running;
...
async fn start_ipc(
    ...
    if stream.is_none() {
        ...
        let mut args = if std::env::var("XIAOCHEN_EMBEDDED_HOST").as_deref() == Ok("1") {
            vec!["--cm", "--xiaochen-embedded-host"]
        } else {
            vec!["--cm"]
        };
        #[allow(unused_mut)]
