...
pub const LAN_V1_CONTROLLER_IP: std::net::Ipv4Addr = std::net::Ipv4Addr::LOCALHOST;

pub fn is_lan_v1_controller_ip(ip: std::net::IpAddr) -> bool {
    match ip {
        std::net::IpAddr::V4(ip) => ip == LAN_V1_CONTROLLER_IP,
        std::net::IpAddr::V6(ip) => ip.to_ipv4_mapped() == Some(LAN_V1_CONTROLLER_IP),
    }
}

pub fn lan_v1_role() -> &'static str {
    let name = std::env::current_exe()
        ...
        .unwrap_or_default();
    if name.contains("controller") || name.contains("control") || name.contains("控制端") {
        "controller"
    } else if name.contains("host") || name.contains("被控端") {
        "host"
    } else {
        ""
    }
...
pub fn is_lan_v1_role() -> bool {
    !lan_v1_role().is_empty()
}
