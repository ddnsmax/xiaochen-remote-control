...
impl RendezvousMediator {
    ...
    pub async fn start_all() {
        let lan_only = crate::common::is_lan_v1_role();
        if !lan_only {
            crate::test_nat_type();
        }
        if !lan_only && config::is_outgoing_only() {
            loop {
                sleep(1.).await;
            }
        }
        ...
        let start_lan_listening = if lan_only {
            crate::common::lan_v1_role() == "host"
        } else {
...
async fn direct_server(server: ServerPtr) {
    ...
    let mut port = 0;
    loop {
        let role = crate::common::lan_v1_role();
        let stopped = option2bool("stop-service", &Config::get_option("stop-service"));
        let disabled = if role == "host" {
            false
        ...
        if let Some(l) = listener.as_mut() {
            ...
            if let Ok(Ok((stream, addr))) = hbb_common::timeout(1000, l.accept()).await {
                let lan_direct = role == "host";
                if lan_direct && !crate::common::is_lan_v1_controller_ip(addr.ip()) {
                    log::warn!("Rejected LAN direct access from {}", addr);
                    continue;
                }
