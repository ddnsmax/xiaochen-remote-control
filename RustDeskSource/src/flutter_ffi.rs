...
pub fn main_get_lan_role() -> SyncReturn<String> {
    SyncReturn(crate::common::lan_v1_role().to_owned())
}

pub fn main_get_lan_controller_status() -> SyncReturn<i32> {
    SyncReturn(crate::lan::controller_status())
}
...
pub fn main_get_peer_option_sync(id: String, key: String) -> SyncReturn<String> {
    if key == "alias" && std::env::var_os("XIAOCHEN_EMBEDDED").is_some() {
        if let Ok(name) = std::env::var("XIAOCHEN_PEER_NAME") {
            if !name.trim().is_empty() {
                return SyncReturn(name);
            }
        }
    }
...
pub fn cm_get_config(name: String) -> String {
    #[cfg(not(target_os = "ios"))]
    {
        if name == "hide_cm" && crate::common::lan_v1_role() == "host" {
