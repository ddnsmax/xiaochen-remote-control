...
const bool _xiaoChenEmbeddedHost = true;
...
class ConnectionManagerState extends State<ConnectionManager>
  ...
  final bool _embeddedHost = _xiaoChenEmbeddedHost;
  int? _embeddedChatClientId;
  ...
  Widget build(BuildContext context) {
    ...
    if (_embeddedHost) {
      return _buildEmbeddedChat(serverModel);
    }
  ...
  Widget _buildEmbeddedChat(ServerModel serverModel) {
    if (serverModel.clients.isEmpty) {
      _embeddedChatClientId = null;
      return Column(
        children: [
          buildTitleBar(),
    ...
    final client = serverModel.clients.first;
    if (_embeddedChatClientId != client.id) {
      _embeddedChatClientId = client.id;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        gFFI.chatModel.changeCurrentKey(MessageKey(client.peerId, client.id));
        client.unreadChatMessageCount.value = 0;
      ...
      children: [
        ...
        Expanded(
          child: Consumer<ChatModel>(
            builder: (_, __, ___) => ChatPage(type: ChatPageType.desktopCM),
          ),
        ),
      ],
    );
