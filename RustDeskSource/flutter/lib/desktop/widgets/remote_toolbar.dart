...
class _RemoteToolbarState extends State<RemoteToolbar> {
  ...
  Widget _buildToolbar(
    ...
    if (Platform.environment['XIAOCHEN_ALLOW_CONTROL_TOGGLE'] == '1') {
      toolbarItems.add(_ViewControlToggle(id: widget.id, ffi: widget.ffi));
    }
...
class _ViewControlToggle extends StatelessWidget {
  final String id;
  final FFI ffi;

  const _ViewControlToggle({Key? key, required this.id, required this.ffi})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    final model = Provider.of<FfiModel>(context);
    final viewOnly = model.viewOnly;
    return _IconMenuButton(
      icon: Icon(
        viewOnly ? Icons.mouse_outlined : Icons.mouse,
        color: Colors.white,
        size: _ToolbarTheme.buttonSize,
      ),
      tooltip: viewOnly ? '允许控制' : '停止控制',
      onPressed: () async {
        await bind.sessionToggleOption(
            sessionId: ffi.sessionId, value: kOptionToggleViewOnly);
        final next = await bind.sessionGetToggleOption(
            sessionId: ffi.sessionId, arg: kOptionToggleViewOnly);
        model.setViewOnly(id, next ?? !viewOnly);
      },
      color: viewOnly ? _ToolbarTheme.inactiveColor : _ToolbarTheme.blueColor,
      hoverColor: viewOnly
