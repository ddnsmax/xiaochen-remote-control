...
import 'package:flutter_hbb/desktop/pages/lan_role_page.dart';
...
void runMainApp(bool startService) async {
  ...
  final lanRole = bind.mainGetLanRole();
  if (lanRole.isEmpty) {
    checkUpdate();
  }
  ...
  // Set window option.
  final windowOptions = lanRole.isEmpty
      ? getHiddenTitleBarWindowOptions(
          isMainWindow: true, alwaysOnTop: alwaysOnTop)
      : WindowOptions(
          center: true,
          skipTaskbar: lanRole == 'host',
          titleBarStyle: TitleBarStyle.normal,
          alwaysOnTop: alwaysOnTop,
        );
  windowManager.waitUntilReadyToShow(windowOptions, () async {
    if (lanRole.isNotEmpty) {
      await windowManager.setPreventClose(false);
    }
    // Restore the location of the main window before window hide or show.
    await restoreWindowPosition(WindowType.Main);
    if (lanRole == 'controller') {
      await windowManager.setSize(const Size(760, 520));
      await windowManager.setMinimumSize(const Size(620, 420));
      await windowManager.setTitle('局域网控制端');
    } else if (lanRole == 'host') {
      await windowManager.setSize(const Size(360, 180));
      await windowManager.setMinimumSize(const Size(360, 180));
      await windowManager.setMaximumSize(const Size(360, 180));
      await windowManager.setResizable(false);
      await windowManager.setTitle('被控端');
    }
    ...
    } else if (lanRole == 'controller') {
      windowManager.setTitle('局域网控制端');
      setResizable(true);
    } else {
      windowManager.setTitle('被控端');
      setResizable(false);
    }
  });
}
...
void runConnectionManagerScreen() async {
  ...
  const embeddedHost = true;
  final hide = !embeddedHost &&
      (bind.mainGetLanRole() == 'host' ||
          await bind.cmGetConfig(name: "hide_cm") == 'true');
  gFFI.serverModel.hideCm = hide;
  if (hide) {
    await hideCmWindow(isStartup: true);
  } else {
    await showCmWindow(isStartup: true, compactChat: embeddedHost);
  }
  setResizable(false);
...
showCmWindow({bool isStartup = false, bool compactChat = false}) async {
  if (isStartup) {
    final initialSize = compactChat
        ? const Size(430, 420)
        : kConnectionManagerWindowSizeClosedChat;
    WindowOptions windowOptions =
        getHiddenTitleBarWindowOptions(size: initialSize, alwaysOnTop: true);
    ...
    await windowManager.setSizeAlignment(initialSize, Alignment.topRight);
